...
export class DesignationComponent implements OnInit {
      ...
      (result) => {
        ...
        result.designations.allDesignations.forEach(element => {
          let designation : DesignationAndPrivilege = {
            designation : element,
            privilege : map.has(element.designationId) ? map.get(element.designationId) : null
          }
        ...
        this.designationWithPrivilege.getPrivilegeByDesignationId(res.designation.designationId).subscribe(
          (privilegeRes : DesignationWithPrivilegeResponse) => {
            ...
            let designation : DesignationAndPrivilege = {
              ...
              privilege : privilegeRes.designationWithPrivilege != null ? privilegeRes.designationWithPrivilege : undefined
            }
            this.allDesignations.push(designation);
    ...
    if(confirm("Are you sure you want to delete this designation ?")){
      ...
      this.designationService.deleteDesignation(id).subscribe(
        (res : DesignationResponse) => {
          ...
          this.toaster.success("Successfully deleted designation - " + res.designation.designationName + " !");
          this.spinnerService.hideSpinner();
        },
        (error) => {
          // console.log(error);
  ...
  removeDesignationFromAllDesignations(allDesignations : DesignationAndPrivilege[], toBeRemoved : DesignationResponse){
    let newArray : DesignationAndPrivilege[] = [];

    for (let i = 0; i < allDesignations.length; i++) {
        if (allDesignations[i].designation.designationId !== toBeRemoved.designation.designationId) {
  ...
  addPrivilegeToDesignation(allDesignations : DesignationAndPrivilege[], designationId : number, privilege : DesignationWithPrivilege){
    for (let i = 0; i < allDesignations.length; i++) {
  ...
  removePrivilege(privilegeId : number){
    ...
    this.designationWithPrivilege.removePrivilege(privilegeId).subscribe(
      (result) => {
        ...
        this.toaster.success("Successfully removed privilege from designation !");
        this.spinnerService.hideSpinner();
      },
      (error) => {
        this.spinnerService.hideSpinner();
  ...
  removePrivilegeFromDesignation(allDesignations : DesignationAndPrivilege[], designationId : number){
    for (let i = 0; i < allDesignations.length; i++) {
        if (allDesignations[i].designation.designationId === designationId) {
