...
export class ResumeComponent {
    ...
    this.resumeService.getResumeByCandidateId(this.userId).subscribe(
      (res) => {
        ...
        if (res.resume == null) {
          ...
          this.resumeUrl =
            this.userResume !== undefined ? this.userResume.resumeUrl : '';
          this.urlSafe = this.sanitizer.bypassSecurityTrustResourceUrl(
            this.resumeUrl !== undefined ? this.resumeUrl : ''
          );
        }
        this.spinner.hideSpinner();
        this.resumeForm
          .get('resumeUrl')
          ?.setValue(this.userResume.resumeUrl || '');
      },
      (error) => {
        this.spinner.hideSpinner();
        console.log(error);
  ...
  add() {
    ...
    this.resumeService
      .addResumeByCandidateId(this.resumeForm.get('resumeUrl')?.value)
      .subscribe(
        (res) => {
          console.log(res);
